...
test.describe('Theme Toggle Button', () => {
  ...
  test('should be visible in the footer', async ({ page }) => {
    ...
    const hasAriaLabel = await themeToggleButton.evaluate(
      (el) => el.hasAttribute('aria-label')
    );
    expect(hasAriaLabel).toBeTruthy();

    // Take a screenshot showing the button in the footer
    await page.screenshot({ path: 'e2e/screenshots/theme-toggle-button-in-footer.png' });
  });
  ...
  test('should show the sun icon in dark mode', async ({ page }) => {
    ...
    // Take a screenshot
    await page.screenshot({ path: 'e2e/screenshots/theme-toggle-dark-mode.png' });
  });
  ...
  test('should show the moon icon in light mode', async ({ page }) => {
    ...
    await page.screenshot({ path: 'e2e/screenshots/theme-toggle-light-mode.png' });
  });

  test('should toggle from dark to light theme when clicked', async ({ page }) => {
    ...
    const isLightTheme = await page.evaluate(() => document.documentElement.classList.contains('light'));
    expect(isLightTheme).toBeTruthy();
    
    // Verify localStorage was updated
    const theme = await page.evaluate(() => localStorage.getItem('scry-ui-theme'));
    expect(theme).toBe('light');
    
    // Take a screenshot after toggle
    await page.screenshot({ path: 'e2e/screenshots/theme-toggled-to-light.png' });
  });

  test('should toggle from light to dark theme when clicked', async ({ page }) => {
    ...
    // Take a screenshot after toggle
    await page.screenshot({ path: 'e2e/screenshots/theme-toggled-to-dark.png' });
  });

  test('should persist theme preference between page loads', async ({ page }) => {
    ...
    const storedTheme = await page.evaluate(() => localStorage.getItem('scry-ui-theme'));
  ...
  test('should be keyboard accessible', async ({ page }) => {
    ...
    await page.screenshot({ path: 'e2e/screenshots/theme-toggle-keyboard-focus.png' });
  ...
  test('should override system theme preference when toggled', async ({ browser }) => {
    ...
    await page.screenshot({ path: 'e2e/screenshots/light-theme-override-system.png' });
