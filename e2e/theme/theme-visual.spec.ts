...
enhancedTest.describe("Theme Visual Appearance", () => {
  ...
  enhancedTest(
    "should apply correct styles in dark theme",
  ...
  enhancedTest(
    "should apply correct styles in light theme",
    async ({ page }: { page: Page }, testInfo: TestInfo) => {
      // Initialize environment for this test
      await initializeDebugEnvironment(testInfo);
      debugLog(`Starting visual test: ${testInfo.title}`);
  ...
  enhancedTest(
    "should have visible CTA button with proper contrast in both themes",
    async ({ browser }: { browser: Browser }, testInfo: TestInfo) => {
      // Initialize environment for this test
      await initializeDebugEnvironment(testInfo);
      debugLog(`Starting visual test: ${testInfo.title}`);
      ...
      const darkCta = darkPage.getByRole("button", {
      ...
      await expectScreenshot(darkPage, testInfo, "theme-dark-cta", {
        ...
        mask: [
          darkPage.locator(
            'body > *:not(:has(button:has-text("Get Early Access")))',
          ),
        ],
      });
      ...
      const lightCta = lightPage.getByRole("button", {
        name: /get early access/i,
      });
  ...
  enhancedTest(
    "should render both themes correctly across different viewports",
    async ({ browser }: { browser: Browser }, testInfo: TestInfo) => {
      // Initialize environment for this test
      await initializeDebugEnvironment(testInfo);
      debugLog(`Starting visual test: ${testInfo.title}`);
