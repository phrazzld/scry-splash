...
export class CtaForm extends BasePage {
  ...
  private readonly errorMessageText =
  ...
  async fillEmail(
    ...
    await waitForFormReadyCI(this.page, testInfo, this.formSelector, {
  ...
  async submit(
    ...
    logger.info(
      `Submit button found - text: "${buttonText}", enabled: ${buttonEnabled}`,
    );
  ...
  async waitForSuccessMessage(
    ...
    try {
      ...
      const message = await this.waitForElement(
      ...
      logger.warn(
        `Data-testid selector failed: ${e instanceof Error ? e.message : String(e)}`,
      );
      logger.info("Trying alternative text-based selector");

      try {
        const textSelector = this.page.getByText(
        ...
        const error =
          textError instanceof Error ? textError : new Error(String(textError));
