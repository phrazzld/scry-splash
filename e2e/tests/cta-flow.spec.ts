...
enhancedTest.describe("CTA Flow @stable", () => {
  ...
  enhancedTest(
    "happy path - successful email submission",
    async ({ page }, testInfo) => {
      ...
      const successMessage = await ctaForm.waitForSuccessMessage(
        testInfo,
        30000,
      );
      ...
      await expectScreenshot(page, testInfo, "cta-flow-success", {
        ...
        mask: [page.locator("body > *:not(:has(.success-message))")],
      });
  ...
  enhancedTest(
    "client-side invalid email validation",
    async ({ page }, testInfo) => {
      ...
      await expectScreenshot(page, testInfo, "cta-flow-invalid-email", {
        ...
        mask: [page.locator("body > *:not(form)")],
      });
  ...
  enhancedTest(
    "mocked server error - displays error message",
    async ({ page }, testInfo) => {
      ...
      await expect(errorMessage).toContainText(
        /Submission failed|error submitting/i,
      );
      ...
      await expectScreenshot(page, testInfo, "cta-flow-error", {
        ...
        mask: [page.locator("body > *:not(:has(.error-message))")],
      });
  ...
  enhancedTest(
    "CTA form should display correctly across different viewports",
    async ({ page }, testInfo) => {
