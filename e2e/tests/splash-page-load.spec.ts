...
enhancedTest.describe("Splash Page Load Tests @stable", () => {
  ...
  enhancedTest(
    "should navigate to the base URL and display main elements",
    async ({ page }, testInfo) => {
  ...
  enhancedModeratelyFlakyTest(
    "should take a visual screenshot of the splash page",
    async ({ page }: { page: Page }, testInfo: TestInfo) => {
      ...
      logger.step(
        "Taking screenshot for visual regression using enhanced visual testing",
      );
  ...
  enhancedModeratelyFlakyTest(
    "should display correctly across different viewports",
    async ({ page }: { page: Page }, testInfo: TestInfo) => {
      const logger = createTestLogger(testInfo.title);
      logger.start();
