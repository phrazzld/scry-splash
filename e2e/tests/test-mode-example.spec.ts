...
performanceTest.describe('@performance Performance Tests', () => {
  performanceTest('should only run when performance testing is enabled', async ({ page }) => {
    ...
    console.log('Performance test is running - performance testing must be enabled');
...
enhancedTest.describe('Test Mode Configuration', () => {
  enhancedTest('should apply correct timeouts based on test tags', async ({ page }, testInfo) => {
    ...
    await expect(page.locator('body')).toBeVisible({ timeout: timeouts.action });
  });
  
  enhancedTest('should have test mode information attached', async ({ page }, testInfo) => {
