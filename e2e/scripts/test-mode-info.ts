...
function displayModeInfo(mode: TestMode) {
  ...
  console.log(
    `   Visual Testing: ${config.visualTestingEnabled ? "✅ Enabled" : "❌ Disabled"}`,
  );
  console.log(
    `   Performance Threshold: ${config.performanceThresholdMultiplier}x`,
  );
  ...
  console.log(
    `   Include Tags: ${config.includeTags.length > 0 ? config.includeTags.join(", ") : "None"}`,
  );
  console.log(
    `   Exclude Tags: ${config.excludeTags.length > 0 ? config.excludeTags.join(", ") : "None"}`,
  );
  console.log(
    `   Screenshots on Failure: ${config.captureScreenshotsOnFailure ? "✅" : "❌"}`,
  );
  console.log(
    `   Screenshots on Success: ${config.captureScreenshotsOnSuccess ? "✅" : "❌"}`,
  );
...
function main() {
  ...
  console.log(
    "   PLAYWRIGHT_TEST_GREP=<pattern>  # Only run tests matching pattern",
  );
  console.log(
    "   PLAYWRIGHT_TEST_GREP_INVERT=<pattern> # Skip tests matching pattern",
  );
