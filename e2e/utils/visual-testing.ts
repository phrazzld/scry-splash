 ...
 */
export function getSnapshotUpdateMode():
  | "all"
  | "missing"
  | "on-failure"
  | undefined {
  ...
  if (existingMode) {
    ...
    if (["all", "missing", "on-failure"].includes(existingMode)) {
      debugLog(
        `Using user-defined snapshot update mode: ${existingMode}`,
        "info",
      );
      return existingMode as "all" | "missing" | "on-failure";
    } else {
      debugLog(
        `Invalid snapshot update mode: ${existingMode}, defaulting to undefined`,
        "warn",
      );
    }
  }
...
export async function setViewport(
  ...
  try {
    ...
    const dimensions =
      typeof viewport === "string" ? viewportDimensions[viewport] : viewport;
...
export async function expectScreenshot(
  ...
  try {
    ...
    if (shouldSkipVisualTests()) {
      debugLog(
        `Skipping visual comparison for "${name}" in CI environment`,
        "info",
      );
      debugLog(
        "To enable visual tests in CI, set VISUAL_TESTS_ENABLED_IN_CI=1",
        "info",
      );
    ...
    try {
      ...
      if (isRunningInCI() && process.env.VISUAL_TESTS_ENABLED_IN_CI === "1") {
        ...
        debugLog(
          `WARNING: Visual comparison failed in CI (update mode: ${updateMode}): ${errorMessage}`,
          "warn",
        );
        debugLog("Available options to fix this issue:", "info");
        debugLog(
          "1. Set VISUAL_TESTS_ENABLED_IN_CI=0 to disable visual tests in CI completely",
          "info",
        );
        debugLog(
          "2. Set PLAYWRIGHT_UPDATE_SNAPSHOTS=all to update all snapshots",
          "info",
        );
        debugLog(
          "3. Set PLAYWRIGHT_UPDATE_SNAPSHOTS=on-failure to update failing snapshots",
          "info",
        );
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    debugLog(
      `Visual comparison failed for "${name}": ${errorMessage}`,
      "error",
    );
...
export async function expectScreenshotForViewports(
  ...
  if (shouldSkipVisualTests()) {
    debugLog(
      `Skipping multi-viewport visual test for "${name}" in CI environment`,
      "info",
    );
    ...
    if (viewports.length > 0 && options.saveDebugScreenshot !== false) {
      debugLog(
        `Taking debug screenshot for reference (using ${viewports[0]} viewport)`,
      );
      await setViewport(page, viewports[0]);
      await takeAndSaveScreenshot(
        testInfo,
        page,
        `debug-ci-skipped-${name}-${viewports[0]}`,
      );
    }
  ...
  debugLog(
    `Running visual tests for "${name}" across ${viewports.length} viewports`,
  );
...
export async function generateBaselineScreenshot(
  ...
  try {
    ...
    if (isRunningInCI()) {
      debugLog(
        `Generating baseline screenshot in CI environment for "${name}"`,
        "warn",
      );
      debugLog(
        "Note: This will override existing snapshots if they exist",
        "warn",
      );
      debugLog(
        "For more controlled updates, consider using PLAYWRIGHT_UPDATE_SNAPSHOTS with expectScreenshot",
        "info",
      );
    } else {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    debugLog(
      `Failed to generate baseline screenshot: ${errorMessage}`,
      "error",
    );
    throw error;
  }
