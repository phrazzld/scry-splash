...
export async function saveCustomArtifact(
  ...
  try {
    ...
    if (typeof content === 'object' && !(content instanceof Buffer) && extension === 'json') {
    ...
    debugLog(`Artifact saved (${elapsedMs}ms, ${stats.size} bytes): ${artifactPath}`);
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    debugLog(`Failed to save artifact ${artifactName}.${extension}: ${errorMessage}`, 'error');
    throw error;
  }
}
...
export async function ensureAndGetCustomSubDir(
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    debugLog(`Failed to create subdirectory ${subDirName}: ${errorMessage}`, 'error');
    throw error;
  }
}
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (entry.isDirectory() && recursive) {
        result += await getDirectoryListing(entryPath, recursive, `${indent}  `);
      } else {
 ...
 * @param testInfo Playwright TestInfo object
 */
export async function initializeDebugEnvironment(testInfo: TestInfo): Promise<void> {
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    debugLog(`Failed to initialize debug environment: ${errorMessage}`, 'error');
    throw error;
  }
}