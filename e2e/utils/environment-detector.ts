...
export function getEnvironmentVariables(): Record<string, string> {
  const relevantVarNames = [
    ...
    'CIRCLE_BRANCH', 'CIRCLE_BUILD_NUM', 'CIRCLE_JOB', 
    'TRAVIS_BUILD_ID', 'TRAVIS_JOB_ID',
    
    // Node-related
    'NODE_ENV', 'NODE_PATH', 'NODE_OPTIONS',
    
    // Playwright-related
    'PLAYWRIGHT_BROWSERS_PATH', 'PWDEBUG', 'PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD',
    
    // Custom project variables
    'NEXT_PUBLIC_FORMSPARK_FORM_ID', 'RUN_ALL_BROWSERS'
  ];
 ...
 */
export function updateBrowserInfo(browserType: BrowserType, browserVersion: string): void {
...
export function isRunningHeadless(): boolean {
  return isRunningInCI() || process.env.HEADLESS === 'true' || process.env.HEADLESS === '1';
}
...
export function printEnvironmentDiagnosis(): void {
  ...
  console.log(`Memory: ${Math.round(env.freeMemory / 1024 / 1024)}MB free / ${Math.round(env.totalMemory / 1024 / 1024)}MB total`);
  ...
  console.log(`Retry failed tests: ${config.retryFailedTests} (max: ${config.maxRetries})`);
