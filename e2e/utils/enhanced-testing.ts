 ...
 */
export const enhancedCITest = mergeTests(
  withErrorReporting,
  withCIDebugReporting,
);
...
export async function waitForElementStability(
  ...
  let lastRect: { x: number; y: number; width: number; height: number } | null =
    null;
...
export async function waitForFormReady(
  ...
  try {
    ...
    if (!formExists) {
      if (debug) {
        logger.error(
          `Form with selector "${formSelector}" not found in the document`,
        );
    ...
    try {
      ...
      const visibleInputLocator = page
        .locator(
          `${formSelector} input:not([name="_gotcha"]):not([tabindex="-1"])`,
        )
        .first();
    ...
    if (debug) {
      logger.error(
        `Failed waiting for form: ${error instanceof Error ? error.message : String(error)}`,
      );
      logger.end("failed");
    }
    throw error;
...
export async function waitForAnimationsComplete(
  ...
  debugLog(
    `Waiting for animations to complete (timeout: ${timeout}ms, retries: ${retries})...`,
  );

  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      debugLog(
        `Checking for animations (attempt ${attempt + 1}/${retries + 1})...`,
      );
      ...
      await page.waitForFunction(
        () => {
          ...
          return (
            animating.length === 0 ||
            Array.from(animating).every((el) => {
              const styles = window.getComputedStyle(el);
              return (
                styles.animationPlayState === "completed" ||
      ...
      if (attempt === retries) {
        ...
        debugLog(
          `Animation check failed on attempt ${attempt + 1}, retrying...`,
          "warn",
        );
      }
    }
      ...
      if (path.startsWith("http://") || path.startsWith("https://")) {
        ...
        if (currentUrl && currentUrl !== "about:blank") {
          ...
          try {
            ...
            debugLog(
              `Could not get baseURL from context, using default: ${e}`,
              "warn",
            );
          }
        }
  ...
  async waitForElement(
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      debugLog(
        `Failed to find element: ${selector} (state: ${state}): ${errorMessage}`,
        "error",
      );
      ...
      await captureDebugInfo(
        this.page,
        testInfo,
        `wait-for-element-failed-${selector.replace(/\W+/g, "-")}`,
      );
...
export async function waitForFormReadyCI(
  ...
  if (debug) {
    logger.start();
    logger.info(
      `Waiting for form with selector: ${formSelector} (timeout: ${timeout}ms)`,
    );
  }
  ...
  try {
    ...
    const visibleInputs = formLocator
      .locator(
        'input:not([tabindex="-1"]):not([name="_gotcha"]), button, select, textarea',
      )
    ...
    if (inputCount > 0) {
      ...
      await visibleInputs
        .first()
        .waitFor({ state: "visible", timeout: timeout / 4 });
    }
    ...
    if (debug) {
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      logger.error(
        `Form readiness check failed: ${errorMessage}`,
        error as Error,
      );
      logger.end("failed");
    }
    ...
    await captureDebugInfo(
      page,
      testInfo,
      `form-not-ready-${formSelector.replace(/\W+/g, "-")}`,
    );
    throw error;
  }
}
...
export async function waitForElementCI(
  ...
  const environmentTimeouts = getEnvironmentTimeouts();
  const { state = "visible", timeout = environmentTimeouts.elementWait } =
    options;
