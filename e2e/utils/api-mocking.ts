 ...
 */
const DEFAULT_MOCK_OPTIONS: Omit<
  Required<FormSparkMockOptions>,
  "networkFailure"
> = {
  success: true,
...
export async function mockFormSparkAPI(
  ...
  await page.route(submitURL, async (route: Route) => {
    ...
    if (config.networkFailure) {
      if (config.enableLogging) {
        console.log(
          `[API Mock] Simulating network failure: ${config.networkFailure}`,
        );
      }
    ...
    if (config.enableLogging) {
      console.log(`[API Mock] Returning response:`, {
        statusCode,
        body: responseBody,
      });
    }
  ...
  return new Promise((resolve) => {
    ...
    const requestHandler = (request: Request) => {
      if (request.url() === submitURL) {
        console.warn(
          `[API Mock] WARNING: Real API call detected to ${submitURL}`,
        );
