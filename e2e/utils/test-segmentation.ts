...
function shouldSkipTest(testInfo: TestInfo): boolean {
  ...
  if (hasTestTag(testInfo, TestTag.Performance) && shouldSkipTestCategory('performance')) {
    return true;
  }
  ...
  if (config.includeTags.length > 0) {
    const hasIncludeTag = config.includeTags.some(tag => hasTestTag(testInfo, tag));
    if (!hasIncludeTag) {
      return true;
    }
  }
 ...
 */
export function shouldSkipCategory(_testInfo: TestInfo, category: 'visual' | 'performance' | 'flaky'): boolean {
