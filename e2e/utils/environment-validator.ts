...
async function validateDirectories(result: ValidationResult): Promise<void> {
  const directoryDetails: Record<string, { exists: boolean; writable: boolean }> = {};
  ...
  for (const dir of REQUIRED_DIRS) {
    try {
      ...
      result.errors.push(`Directory validation failed for "${dir}": ${(error as Error).message}`);
  ...
  if (process.env.CI) {
    try {
      ...
      result.warnings.push(`Failed to get directory listing: ${(error as Error).message}`);
    }
  }
}
 ...
 */
export async function validateTestEnvironment(testInfo: TestInfo): Promise<void> {
  // Run validation
...
export function ensureTestArtifactDirectory(testInfo: TestInfo): string {
  ...
  const testArtifactDir = path.join(ARTIFACTS_DIR, sanitizeFilename(testInfo.title));
