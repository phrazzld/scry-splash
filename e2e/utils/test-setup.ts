...
export async function capturePageState(
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    debugLog(
      `Error capturing page state for context "${context}": ${errorMessage}`,
      "error",
    );
  }
}
...
export function setupConsoleLogging(
  ...
  return {
    ...
    save: async () => {
      ...
      return saveCustomArtifact(testInfo, "console-logs", content, "log");
    },
  };
}
...
export async function attachDebugArtifacts(
  ...
  try {
    ...
    const errorMessage = error instanceof Error ? error.message : String(error);
    debugLog(
      `Error attaching debug artifacts for context "${context}": ${errorMessage}`,
      "error",
    );
  }
}
