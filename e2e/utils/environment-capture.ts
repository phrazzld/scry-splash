...
export async function captureFailureInfo(
  ...
  const failureInfo: FailureInfo = {
    ...
    testMetadata: {
      ...
      duration:
        Date.now() - ((testInfo as any).startTime?.getTime() || Date.now()),
    },
  ...
  const locationMatch = error.stack?.match(
    /at.+?(\w+\.(?:spec|test)\.[jt]sx?):(\d+):(\d+)/,
  );
  if (locationMatch) {
...
export function classifyFailure(error: Error): string {
  ...
  if (
    errorText.includes("Cannot find element") ||
  ...
  if (
    errorText.includes("navigation failed") ||
    errorText.includes("page.goto")
  ) {
    return FailureType.Navigation;
  }

  if (
    errorText.includes("click") ||
    errorText.includes("type") ||
    errorText.includes("press") ||
  ...
  if (
    ...
    errorText.includes("CI environment") ||
 ...
 */
export async function captureEnvironmentDiagnostics(
  testInfo: TestInfo,
): Promise<string> {
  // Get environment information
  const envInfo = getEnvironmentInfo();
  ...
  if (envInfo.isCI) {
    try {
      ...
      console.warn(
        `Error collecting CI diagnostics: ${(error as Error).message}`,
      );
    }
  }
 ...
 */
export async function captureArtifactInfo(
  testInfo: TestInfo,
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (file.includes("screenshot") && file.endsWith(".png")) {
        ...
        file.includes("trace") &&
        (file.endsWith(".zip") || file.endsWith(".trace"))
      ) {
...
export async function createFailureReport(
  ...
  const reportPath = path.join(
    reportDir,
    `failure-report-${failureInfo.id}.html`,
  );
...
export function setupErrorCapture(
  page: Page,
  testInfo: TestInfo,
): (error: Error, stepName?: string) => Promise<void> {
  return async (error: Error, stepName?: string) => {
    try {
      ...
      // Capture failure information
      const failureInfo = await captureFailureInfo(
        error,
        page,
        testInfo,
        stepName,
