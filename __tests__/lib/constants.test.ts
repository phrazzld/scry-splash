import { COLORS, FONT_SIZES, FONT_WEIGHTS, SPACING, FORMSPARK } from '@/lib/constants';
...
describe('Constants', () => {
  ...
  describe('FORMSPARK', () => {
    ...
    it('should generate correct submit URL', () => {
      expect(FORMSPARK.SUBMIT_URL).toBe(`${FORMSPARK.FORM_ACTION}${FORMSPARK.FORM_ID}`);
    ...
    it('should generate submit URL dynamically with custom form ID', () => {
      ...
      expect(customFormSpark.SUBMIT_URL).toBe('https://submit-form.com/custom-id');
    });
  });
});