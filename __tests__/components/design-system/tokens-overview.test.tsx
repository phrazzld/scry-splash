...
describe('TokensOverview Component', () => {
  it('renders without errors', () => {
    ...
    expect(screen.getByText(/A comprehensive guide to design tokens, components, and patterns/)).toBeInTheDocument();
  });
  ...
  it('renders category descriptions correctly', () => {
    ...
    expect(screen.getByText('Brand colors and semantic color tokens')).toBeInTheDocument();
    expect(screen.getByText('Font sizes, weights, and line heights')).toBeInTheDocument();
    expect(screen.getByText('Spacing scale and layout measurements')).toBeInTheDocument();
    expect(screen.getByText('Duration, timing functions, and transitions')).toBeInTheDocument();
    expect(screen.getByText('Grid system and container sizing')).toBeInTheDocument();
    expect(screen.getByText('Integration with shadcn/ui component library')).toBeInTheDocument();
  });
  ...
  it('renders view links for all categories', () => {
    ...
    expect(links[0]).toHaveAttribute('href', '?path=/story/design-system-colortokens--default');
  });
  ...
  it('renders color examples for Colors category', () => {
    ...
    if (colorsCard) {
      const colorCircles = colorsCard.querySelectorAll('.h-12.w-12.rounded-full');
  ...
  it('correctly maps through token categories array', () => {
    ...
    const descriptions = screen.getAllByText(/tokens|sizes|scale|functions|system|library/);
  ...
  it('passes additional props to the container', () => {
    render(<TokensOverview data-testid="tokens-overview" aria-label="Design system overview" />);
