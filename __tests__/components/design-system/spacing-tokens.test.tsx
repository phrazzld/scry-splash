...
describe('SpacingTokens Component', () => {
  it('renders without errors', () => {
    ...
    expect(screen.getByText(/Scry uses an 8px grid system as the foundation for all spacing/)).toBeInTheDocument();
  });
  ...
  it('renders usage examples with code snippets', () => {
    ...
    expect(screen.getByText(/Paragraph with bottom margin/)).toBeInTheDocument();

    expect(screen.getByText(/\/\* Container with Gap \*\//)).toBeInTheDocument();
    expect(screen.getByText(/Container gap="md"/)).toBeInTheDocument();
    expect(screen.getByText(/GridItem span=\{12\} md=\{6\}>Column 1/)).toBeInTheDocument();
    expect(screen.getByText(/GridItem span=\{12\} md=\{6\}>Column 2/)).toBeInTheDocument();

    expect(screen.getByText(/\/\* Using Special Spacing \*\//)).toBeInTheDocument();
    expect(screen.getByText(/main className="my-vertical-lg"/)).toBeInTheDocument();
    expect(screen.getByText(/Content with vertical margin/)).toBeInTheDocument();
  });
  ...
  it('correctly maps through container gaps array', () => {
    ...
    const gapProps = ['gap="none"', 'gap="sm"', 'gap="md"', 'gap="lg"', 'gap="xl"'];
  ...
  it('passes additional props to the container', () => {
    render(<SpacingTokens data-testid="spacing-tokens" aria-label="Spacing documentation" />);
    const spacingTokensContainer = screen.getByTestId('spacing-tokens');
    expect(spacingTokensContainer).toHaveAttribute('aria-label', 'Spacing documentation');
  });
