...
describe('LayoutTokens Component', () => {
  it('renders without errors', () => {
    ...
    expect(screen.getByText(/Scry uses a responsive 12-column grid system/)).toBeInTheDocument();
  });
  ...
  it('renders the grid visualization section', () => {
    ...
    expect(screen.getByText(/The grid divides the available width into 12 equal columns/)).toBeInTheDocument();
  });
  ...
  it('renders responsive layout examples', () => {
    ...
    expect(screen.getByText('Default (Mobile): Full Width')).toBeInTheDocument();
    expect(screen.getByText('Medium Breakpoint: Two Columns')).toBeInTheDocument();
    expect(screen.getByText('Large Breakpoint: Three Columns')).toBeInTheDocument();
  ...
  it('renders breakpoints table with correct data', () => {
    ...
    expect(screen.getByText('Mobile devices, portrait orientation')).toBeInTheDocument();
    ...
    expect(screen.getByText('Extra large devices, large desktops')).toBeInTheDocument();
    ...
    expect(screen.getByText('Very large devices and monitors')).toBeInTheDocument();
  });
  ...
  it('renders container maximum width table with correct data', () => {
    ...
    expect(screen.getByText('640px')).toBeInTheDocument();
    expect(screen.getByText('Small container for compact content')).toBeInTheDocument();

    expect(screen.getAllByText('lg')).not.toHaveLength(0);
    expect(screen.getByText('1024px')).toBeInTheDocument();
    expect(screen.getByText('Large container for expanded content')).toBeInTheDocument();

    expect(screen.getByText('full')).toBeInTheDocument();
    expect(screen.getByText('100%')).toBeInTheDocument();
    expect(screen.getByText('Full width container with no maximum')).toBeInTheDocument();
  ...
  it('renders container padding table with correct data', () => {
    ...
    // Check section heading
    expect(screen.getByRole('heading', { name: 'Padding' })).toBeInTheDocument();
    ...
    expect(screen.getByText('Adjusts automatically based on screen size')).toBeInTheDocument();
  });
  ...
  it('correctly maps through container maxWidths array', () => {
    ...
    const containerMaxWidths = ['640px', '768px', '1024px', '1280px', '1400px', '100%'];
  ...
  it('correctly maps through container padding array', () => {
    ...
    const containerPadding = ['0', '1rem (16px)', '1.5rem (24px)', '2rem (32px)', '3rem (48px)', 'Varies by breakpoint'];
  ...
  it('passes additional props to the container', () => {
    render(<LayoutTokens data-testid="layout-tokens" aria-label="Layout documentation" />);
    const layoutTokensContainer = screen.getByTestId('layout-tokens');
    expect(layoutTokensContainer).toHaveAttribute('aria-label', 'Layout documentation');
  });
