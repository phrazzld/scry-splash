...
describe('ShadcnIntegration Component', () => {
  it('renders without errors', () => {
    ...
    expect(screen.getByText(/Scry's design system builds on shadcn\/ui/)).toBeInTheDocument();
  });
  ...
  it('renders color mapping table correctly', () => {
    ...
    expect(screen.getByText('--background, --card, --popover')).toBeInTheDocument();
    ...
    expect(screen.getByText('--foreground, --card-foreground, --popover-foreground')).toBeInTheDocument();
    ...
    expect(screen.getByText('Primary buttons and interactive elements')).toBeInTheDocument();
  });
  ...
  it('renders component customization table correctly', () => {
    ...
    expect(screen.getByText("Added 'cta' variant, customized colors and sizes")).toBeInTheDocument();
    expect(screen.getByText('CTAs, form submissions, interactive elements')).toBeInTheDocument();
    
    expect(screen.getByText('Typography')).toBeInTheDocument();
    expect(screen.getByText("Customized with Scry's typography scale and IBM Plex Sans")).toBeInTheDocument();
    expect(screen.getByText('All text elements in the application')).toBeInTheDocument();
    
    expect(screen.getByText('Container')).toBeInTheDocument();
    expect(screen.getByText('Added 12-column grid support and responsive utilities')).toBeInTheDocument();
    expect(screen.getByText('Page layout and component positioning')).toBeInTheDocument();
  });
  ...
  it('renders button component examples', () => {
    ...
    expect(screen.getByText('The default button variant using primary colors.')).toBeInTheDocument();
    expect(screen.getByText('Custom CTA variant designed for primary actions.')).toBeInTheDocument();
    expect(screen.getByText('Secondary button for less prominent actions.')).toBeInTheDocument();
    expect(screen.getByText('Outline button for tertiary actions.')).toBeInTheDocument();
  });
  ...
  it('renders implementation details section correctly', () => {
    ...
    expect(screen.getByText(/import { cva } from "class-variance-authority"/)).toBeInTheDocument();
    expect(screen.getByText(/export const buttonVariants = cva/)).toBeInTheDocument();
    expect(screen.getByText(/"bg-cobalt text-chalk hover:bg-cobalt-light"/)).toBeInTheDocument();
  ...
  it('renders usage guidelines section correctly', () => {
    ...
    // Check do guidelines
    expect(screen.getByText("Use shadcn/ui components with Scry's customizations")).toBeInTheDocument();
    expect(screen.getByText("Follow shadcn/ui's documentation for component APIs")).toBeInTheDocument();
    expect(screen.getByText("Use semantic color variables (--primary, --background) instead of direct brand colors")).toBeInTheDocument();
    expect(screen.getByText("Check Storybook for examples of component usage")).toBeInTheDocument();
    
    // Check don't guidelines
    expect(screen.getByText("Import components directly from shadcn/ui repositories")).toBeInTheDocument();
    expect(screen.getByText("Override component styling with inline styles")).toBeInTheDocument();
    expect(screen.getByText("Create new variants without documenting them")).toBeInTheDocument();
    expect(screen.getByText("Hard-code color values that are already available as variables")).toBeInTheDocument();
  });
  ...
  it('applies custom className when provided', () => {
    const { container } = render(<ShadcnIntegration className="custom-class" />);
    expect(container.firstChild).toHaveClass('custom-class');
  });
  
  it('passes additional props to the container', () => {
    render(<ShadcnIntegration data-testid="shadcn-integration" aria-label="Shadcn integration documentation" />);
    const container = screen.getByTestId('shadcn-integration');
    expect(container).toHaveAttribute('aria-label', 'Shadcn integration documentation');
  });
