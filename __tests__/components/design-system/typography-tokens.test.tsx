...
describe('TypographyTokens Component', () => {
  it('renders without errors', () => {
    ...
    expect(screen.getByText(/Scry uses IBM Plex Sans as its primary typeface/)).toBeInTheDocument();
  });
  ...
  it('renders font family section correctly', () => {
    ...
    expect(screen.getByText(/A modern sans-serif typeface with excellent readability/)).toBeInTheDocument();
  ...
  it('renders font sizes section correctly', () => {
    ...
    expect(screen.getByText('Display - The quick brown fox jumps over the lazy dog')).toBeInTheDocument();
    expect(screen.getByText('Heading - The quick brown fox jumps over the lazy dog')).toBeInTheDocument();
    expect(screen.getByText('Subheading - The quick brown fox jumps over the lazy dog')).toBeInTheDocument();
    expect(screen.getByText('Body - The quick brown fox jumps over the lazy dog')).toBeInTheDocument();
  ...
  it('renders font weights section correctly', () => {
    ...
    expect(screen.getByText('Used for subheadings and emphasis')).toBeInTheDocument();
    expect(screen.getByText('Used for headings and strong emphasis')).toBeInTheDocument();
  });
  ...
  it('renders typography components table correctly', () => {
    ...
    // Check descriptions
    expect(screen.getByText('Used for headings with configurable styles')).toBeInTheDocument();
    expect(screen.getByText('Used for subheadings with configurable styles')).toBeInTheDocument();
    expect(screen.getByText('Used for body text with configurable styles')).toBeInTheDocument();
    
    // Check examples
    expect(screen.getByText('<HeadingText>Heading Example</HeadingText>')).toBeInTheDocument();
    expect(screen.getByText('<SubheadingText>Subheading Example</SubheadingText>')).toBeInTheDocument();
    expect(screen.getByText('<BodyText>Body text example</BodyText>')).toBeInTheDocument();
  ...
  it('renders usage examples with code snippets', () => {
    ...
    // Check code examples
    expect(screen.getByText(/\/\* Using Typography Components \*\//)).toBeInTheDocument();
    expect(screen.getByText(/import { HeadingText, SubheadingText, BodyText } from/)).toBeInTheDocument();
    expect(screen.getByText(/<HeadingText>Main Heading<\/HeadingText>/)).toBeInTheDocument();
    expect(screen.getByText(/<SubheadingText>Subheading<\/SubheadingText>/)).toBeInTheDocument();
    expect(screen.getByText(/<BodyText>This is body text content...<\/BodyText>/)).toBeInTheDocument();
    
    expect(screen.getByText(/\/\* Using CSS Classes \*\//)).toBeInTheDocument();
    expect(screen.getByText(/<h1 className="text-display font-bold">Main Heading<\/h1>/)).toBeInTheDocument();
    expect(screen.getByText(/<h2 className="text-subheading font-medium">Subheading<\/h2>/)).toBeInTheDocument();
    expect(screen.getByText(/<p className="text-body font-regular">This is body text content...<\/p>/)).toBeInTheDocument();
  });
  ...
  it('correctly maps through typography components array', () => {
    ...
    const componentCells = screen.getAllByRole('cell', { name: new RegExp(`^(${componentNames.join('|')})$`) });
  ...
  it('passes additional props to the container', () => {
    render(<TypographyTokens data-testid="typography-tokens" aria-label="Typography documentation" />);
    const typographyTokensContainer = screen.getByTestId('typography-tokens');
    expect(typographyTokensContainer).toHaveAttribute('aria-label', 'Typography documentation');
  });
