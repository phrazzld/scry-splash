...
describe('TypographyTokens Component', () => {
  ...
  it('renders all typography tokens', () => {
    ...
    expect(screen.getByText(/Body text which is used for regular paragraphs/)).toBeInTheDocument();
  });
  ...
  it('applies correct inline styles to typography token examples', () => {
    ...
    const bodyText = screen.getByText(/Body text which is used for regular paragraphs/);
    ...
    styledDivs.forEach(div => {
      if (div.textContent?.includes('Display Text') || div.textContent?.includes('Body text')) {
