...
describe('ColorTokens Component', () => {
  ...
  it('renders color swatches with correct background colors', () => {
    ...
    const inkColorSwatch = inkHex.closest('.flex.flex-col')?.querySelector('.w-full.h-16');
    expect(inkColorSwatch).toHaveStyle({ backgroundColor: 'rgb(18, 18, 18)' });

    const chalkHex = screen.getByText('#FAFAFA');
    const chalkColorSwatch = chalkHex.closest('.flex.flex-col')?.querySelector('.w-full.h-16');
    expect(chalkColorSwatch).toHaveStyle({ backgroundColor: 'rgb(250, 250, 250)' });
  });
  ...
  it('uses correct text colors for contrast', () => {
    ...
    const purpleColorSwatch = purpleHex.closest('.flex.flex-col')?.querySelector('.w-full.h-16');
