...
export function ColorTokens({ className, ...props }: ColorTokensProps) {
  ...
  const brandColors = [
    { 
      ...
      variable: "--color-ink", 
      value: "#121212",
      description: "Primary dark color, used for backgrounds and text in light mode",
    ...
    { 
      ...
      value: "#0047AB",
      description: "Primary accent color, used for interactive elements and emphasis",
    ...
    <div className={cn("w-full p-8", className)} {...props}>
      <div className="mb-8">
        ...
        <BodyText className="mt-2 text-muted-foreground">
          The Scry color system is built around a high-contrast palette featuring Ink (dark), Chalk (light),
          and Cobalt (accent). All colors meet WCAG AA accessibility standards for their intended use.
        </BodyText>
      </div>
      
      {/* Brand Colors */}
      <div className="mb-12">
        <SubheadingText as="h2" className="mb-4">Brand Colors</SubheadingText>
        <Container gap="md">
                ...
                <div className="p-4 bg-card">
                  <div className="flex flex-col gap-1">
                    <div className="flex justify-between">
                      ...
                      <code className="text-body-size font-mono">{color.variable}</code>
                    </div>
                    <div className="flex justify-between">
                      <BodyText weight="medium">HEX:</BodyText>
                      <code className="text-body-size font-mono">{color.value}</code>
                    </div>
                    {color.contrastWithChalk && (
                      <div className="flex justify-between">
                        <BodyText weight="medium">Contrast with Chalk:</BodyText>
                  ...
                  <BodyText className="mt-3 text-muted-foreground">{color.description}</BodyText>
                </div>
              </div>
            </GridItem>
          ))}
      ...
      {/* Semantic Colors */}
      <div className="mb-12">
        <SubheadingText as="h2" className="mb-4">Semantic Colors (shadcn/ui Integration)</SubheadingText>
        <BodyText className="mb-6">
          Scry maps its brand colors to shadcn/ui semantic color variables to ensure consistent theming
          ...
          <tbody>
            {semanticColors.map((color, index) => (
              <tr key={index} className={index % 2 === 0 ? "bg-card" : "bg-muted/30"}>
                ...
                <td className="border border-border p-2 font-mono text-sm">{color.variable}</td>
                <td className="border border-border p-2 font-mono text-sm">{color.value}</td>
                <td className="border border-border p-2">{color.description}</td>
              </tr>
            ))}
          </tbody>
        </table>
          ...
          <GridItem span={12} md={6}>
            <div className="p-4 rounded-md border border-border">
              ...
              <ul className="list-disc pl-5 space-y-2">
                <li>Use Ink for dark backgrounds and Chalk for light backgrounds</li>
                <li>Use Cobalt for interactive elements like buttons and links</li>
                <li>Ensure text has sufficient contrast with its background</li>
                <li>Use semantic color variables in components for consistency</li>
              </ul>
            </div>
          </GridItem>
      ...
      {/* Code Examples */}
      <div>
        <SubheadingText as="h2" className="mb-4">Code Examples</SubheadingText>
