    ...
    <div className={cn("w-full p-8", className)} {...props}>
      <div className="mb-8">
        ...
        <BodyText className="mt-2 text-muted-foreground">
          A comprehensive guide to design tokens, components, and patterns used
          in the Scry application.
        </BodyText>
      </div>
        ...
        {tokenCategories.map((category, index) => (
          <GridItem key={index} span={12} md={6} lg={4} className="mb-8">
            <div className="p-6 rounded-lg border border-border">
              <SubheadingText as="h2" className="mb-2">
                {category.name}
              </SubheadingText>
              ...
              {category.name === "Typography" && (
                <div className="flex flex-col gap-1 mb-4">
                  {category.examples.map((example, i) => {
                    ...
                    if ("style" in example) {
                      return (
                        <div key={i} className={example.style}>
                          {example.name}
                        </div>
                      );
                    }
              ...
              {category.examples.some(
                (ex) => "visual" in ex && ex.visual === "grid-demo",
              ) && (
              ...
              {category.examples.some(
                (ex) => "visual" in ex && ex.visual === "fade-demo",
              ) && (
