...
export interface AnimationTokensProps extends React.HTMLAttributes<HTMLDivElement> {
  /**
    ...
    <div className={cn("w-full p-8", className)} {...props}>
      <div className="mb-8">
        ...
        <BodyText className="mt-2 text-muted-foreground">
          Scry uses subtle animations to enhance the user experience. The animation system provides
          consistent timing and easing functions to ensure smooth transitions throughout the interface.
        </BodyText>
      </div>
      
      {/* Animation Tokens */}
      <div className="mb-12">
        <SubheadingText as="h2" className="mb-4">Animation Tokens</SubheadingText>
          ...
          <tbody>
            {animationTokens.map((token, index) => (
              <tr key={index} className={index % 2 === 0 ? "bg-card" : "bg-muted/30"}>
                <td className="border border-border p-2 font-medium">{token.name}</td>
                <td className="border border-border p-2 font-mono text-sm">{token.variable}</td>
                <td className="border border-border p-2">{token.value}</td>
                <td className="border border-border p-2">{token.description}</td>
              </tr>
            ))}
          </tbody>
        </table>
        
        <SubheadingText as="h3" className="mb-4">Animation Examples</SubheadingText>
        <Container gap="md">
          <GridItem span={12} md={6} className="mb-6">
            <div className="p-6 border border-border rounded-lg h-full">
              <SubheadingText as="h4" className="mb-4">Fade In</SubheadingText>
              <button 
          ...
          <GridItem span={12} md={6} className="mb-6">
            <div className="p-6 border border-border rounded-lg h-full">
              <SubheadingText as="h4" className="mb-4">Button Interactions</SubheadingText>
              <div className="space-y-4">
                <div>
                  <BodyText weight="medium" className="mb-2">Hover State</BodyText>
                  <button className="px-4 py-2 bg-primary text-primary-foreground rounded-md w-full hover:bg-primary/90 transition-colors">
                    Hover me
                  </button>
                </div>
                <div>
                  <BodyText weight="medium" className="mb-2">Active State</BodyText>
      ...
      {/* Animation Classes */}
      <div className="mb-12">
        <SubheadingText as="h2" className="mb-4">Animation Classes</SubheadingText>
          ...
          {animationClasses.map((animClass, index) => (
            <div key={index} className="border border-border rounded-lg overflow-hidden">
              ...
              <div className="p-4">
                ...
                <BodyText weight="medium" className="text-sm mt-4 mb-2">CSS Implementation:</BodyText>
      ...
      {/* Keyframes */}
      <div className="mb-12">
        <SubheadingText as="h2" className="mb-4">Keyframes</SubheadingText>
